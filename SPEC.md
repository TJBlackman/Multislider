...
| `maxClones` | `number` | `600` | Cap on cloned slide elements added to satisfy the loop guard. `0` disables cloning; the guard then falls back to clamped mode. |
...
- `next`/`prev`/page: compute the target head index, then tween to the measured `slides[target].start` via the shortest wrapped delta in the step's direction (never by summed sizes, so float error cannot accumulate and a fractional offset self-realigns on the next step). An offset within half a pixel of a boundary counts as sitting on it; a step of `count >= total` animates one full revolution. Marquee: `offset += speed * dt` inside the rAF loop. Drag: `offset = offsetAtPointerDown - dx`, then momentum and snap (snap takes the shortest wrapped path).
